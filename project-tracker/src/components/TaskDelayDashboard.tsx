...
export const TaskDelayDashboard: React.FC = () => {
      ...
      const subtasks: SubtaskDelay[] = group.subtasks
        ...
        .map(st => {
          ...
          return {
            ...
            assignedTo: st.assignedUser?.email || null,
            milestones,
            averageDelay,
            worstDelay
          };
  ...
  const getDelayColor = (worstDelay: number) => {
    if (worstDelay === 0) return 'bg-green-600';
    ...
    <div className={`${theme.bg} ${theme.text} min-h-screen p-6`}>
      <div className="max-w-7xl mx-auto">
        ...
        {taskDelays.length === 0 ? (
          ...
          <>
            <div className={`${theme.cardBg} rounded-lg p-6 border-2 border-blue-500 mb-6`}>
            ...
            {taskDelays.map((task, idx) => (
              <div key={idx} className={`${theme.cardBg} rounded-lg p-6 border-2 ${
                ...
                <div className="space-y-4">
                  {task.subtasks.map((subtask, subIdx) => (
                    <div key={subIdx} className={`border-2 rounded-lg p-4 ${
